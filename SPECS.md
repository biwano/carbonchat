...
- **Administration Panel** (renamed from "Settings"): Section for knowledge management at `/admin`:
  ...
  - **Documents Management**: CRUD with behavior that depends on the selected document type's `ai` flag.
    - For **AI document types** (`ai = true`): one-click "Research Now" / "Refresh Content" buttons trigger AI-powered scraping. The create dialog asks for **name**, **document type**, and **subject**; content is generated by the AI based on the subject's content. The edit dialog allows updating **name**, **document type**, **subject**, and **content** (manual content editing). Manual edits to `content` are saved directly via the Supabase client and bump `updated_at`. Refreshing overwrites the current `content` with freshly synthesized knowledge.
    - For **non-AI document types** (`ai = false`): documents are purely manual. The create dialog asks for **name**, **document type**, and **content** (the content field is editable at creation time). The `subject` field is **not used** (it is hidden or disabled in both create and edit dialogs). The "Research Now" / "Refresh Content" action is not offered for these documents.
...
- **Document Scraper**: `POST /api/documents/scrape` endpoint using OpenRouter to research `search_query` and synthesize per `document_type` rules. Supports both initial creation and updating (refreshing) existing documents. Only applicable to documents whose type has `ai = true`; the endpoint rejects requests targeting documents of non-AI types.
...
### Key Components
1. **Document Scraper Service**: `POST /api/documents/scrape` endpoint that uses OpenRouter (with search capabilities) to research a topic based on a `subject`'s content and synthesize knowledge according to a `document_type`'s `transformation_instructions`.

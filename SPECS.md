...
- `documents`: Individual knowledge entries that reference a document type, define a `search_query` (for AI types) or hold user-authored `content` (for non-AI types).

For **AI document types** (`ai = true`), an API endpoint triggers an AI agent (via OpenRouter) to perform web research and generate knowledge according to the selected document type's rules. For **non-AI document types** (`ai = false`), documents are purely manual: the user writes the `content` directly and the `search_query` is not used.
...
- **Administration Panel** (renamed from "Settings"): Section for knowledge management at `/admin`:
  ...
  - **Documents Management**: CRUD with behavior that depends on the selected document type's `ai` flag.
    - For **AI document types** (`ai = true`): one-click "Research Now" / "Refresh Content" buttons trigger AI-powered scraping. The create dialog asks for **name**, **document type**, and **search query**; content is generated by the AI. The edit dialog allows updating **name**, **document type**, **search query**, and **content** (manual content editing). Manual edits to `content` are saved directly via the Supabase client and bump `updated_at`. Refreshing overwrites the current `content` with freshly synthesized knowledge.
    - For **non-AI document types** (`ai = false`): documents are purely manual. The create dialog asks for **name**, **document type**, and **content** (the content field is editable at creation time). The `search_query` field is **not editable** (it is hidden or disabled in both create and edit dialogs) and defaults to an empty string. The "Research Now" / "Refresh Content" action is not offered for these documents.
...
- search_query (text)            -- What the AI should research on the internet (empty string for non-AI types)
